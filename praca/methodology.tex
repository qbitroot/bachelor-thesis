...
\begin{enumerate}
    \item \textbf{Valid capsule types} in their canonical VarInt encoding. For example, \texttt{CLOSE\-\_SESSION} is encoded as \texttt{0x6843} (the canonical 2-byte VarInt for value \texttt{0x2843}), and \texttt{DRAIN\-\_SESSION} as \texttt{0x800078ae} (the canonical 4-byte VarInt for \texttt{0x78AE}).
...
Other scenarios include \texttt{all\-\_transports\-\_then\-\_close} (all three transport paths then teardown), \texttt{capsule\-\_between\-\_bidi} (inject a \texttt{MAX\_DATA} capsule between two echo exchanges), \texttt{drain\-\_mid\-\_data} (drain while streams are active), \texttt{rapid\-\_bidi\-\_close} (rapid stream creation before close), and \texttt{kitchen\-\_sink} (all six action types for maximum permutation surface). The full list of 12 scenarios is in Appendix~\ref{appendix:scenarios}.
...
\begin{verbatim}
  ...
  3. capsule(c00000000000284300) -- fuzzed: [Type=overlong CLOSE][Len=0]
\end{verbatim}
