...
WebTransport uses HTTP/3 only for the initial session handshake. Once a session is established, applications have nearly direct access to QUIC streams. The layering can be summarized as follows:

\begin{itemize}
    \item \textbf{Application layer} --- the WebTransport API~\cite{webtransport_api} exposed to web applications
    \item \textbf{HTTP/3 framing} --- used for the initial CONNECT handshake and for capsule exchange on the CONNECT stream
    \item \textbf{QUIC transport} --- provides streams, datagrams, flow control, congestion control, and encryption
    \item \textbf{UDP} --- the underlying network transport
\end{itemize}

\subsection{Session Establishment}

A WebTransport session is initiated by a client sending an extended CONNECT request~\cite{RFC8441} over HTTP/3. The server indicates support for WebTransport by sending a WebTransport-specific setting in its HTTP/3 SETTINGS frame. Both endpoints must also negotiate support for HTTP/3 datagrams and the Capsule Protocol. If the server accepts the CONNECT request (responding with a 2xx status code), the WebTransport session is established. The resulting HTTP/3 stream is called the \emph{CONNECT stream}, and its stream ID serves as the \emph{Session ID} that uniquely identifies the session within the connection.
...
\begin{itemize}
    \item \textbf{Draft-03} (July 2022): an early version that uses the \texttt{SETTINGS\_ENABLE\_WEBTRANSPORT} setting (value \texttt{0x2b603742}), does not define session-level flow control capsules, and uses the upgrade token \texttt{webtransport}. Session termination uses the \texttt{CLOSE\_WEBTRANSPORT\_SESSION} capsule (type \texttt{0x2843}).
...
Fuzzers are typically classified along two axes:

\begin{itemize}
    \item \textbf{By input generation strategy}: \emph{generation-based} fuzzers construct inputs from a protocol specification or grammar, while \emph{mutation-based} fuzzers modify existing valid inputs. Hybrid approaches combine both.
    \item \textbf{By level of program knowledge}: \emph{black-box} fuzzers treat the target as an opaque system with no access to source code or internal state; \emph{white-box} fuzzers use instrumentation (e.g., code coverage, address sanitizers) to guide input generation; \emph{gray-box} fuzzers use partial knowledge, such as protocol specifications, without requiring source-level instrumentation.
\end{itemize}
...
The oneshot mode sends a single malformed capsule per connection on the CONNECT stream. The capsule type, length field, and payload are independently fuzzed using:

\begin{itemize}
    \item Interesting boundary values (0, 1, $2^{n}-1$, $2^{n}$, maximum VarInt values)
    \item Overlong QUIC variable-length integer encodings (e.g., encoding 0 using 2, 4, or 8 bytes)
    \item Cross-layer type confusion: injecting QUIC frame type bytes or HTTP/3 frame type bytes as capsule type values
    \item Truncated and oversized payloads relative to the declared length
\end{itemize}
...
The multistep mode executes multi-step scenarios that interleave legitimate data operations with capsule sequences on a single live session. It is designed to trigger state-machine bugs: use-after-free, state confusion after session close, crash-on-reorder, and similar defects. Each scenario is a list of steps, where each step is one of:

\begin{itemize}
    \item \texttt{bidi(payload)} --- opens a bidirectional WebTransport stream, sends the payload, awaits an echo response (with a 1-second timeout; timeout is non-fatal)
    \item \texttt{uni(payload)} --- opens a unidirectional stream and sends the payload
    \item \texttt{datagram(payload)} --- sends a QUIC datagram
    \item \texttt{capsule(data)} --- sends a raw capsule on the CONNECT stream
    \item \texttt{sleep(duration)} --- pauses between steps
\end{itemize}

From each base scenario, the \emph{mutator} generates all interesting variants:

\begin{itemize}
    \item All permutations (reordering of steps)
    \item Each step duplicated in place and at end of the sequence
    \item Each step omitted
    \item Rapid-fire repetition: same step repeated 2, 5, and 10 times
    \item Reversed order
    \item Prohibited capsule injection at every position (\texttt{WT\_MAX\_STREAM\_DATA} / \texttt{WT\_STREAM\_DATA\_BLOCKED}, which per draft-15 \S5.4 must cause a session error)
    \item Post-CLOSE activity: sending data (bidi/uni/datagram/capsule) after a \texttt{CLOSE\_SESSION} capsule
\end{itemize}
...
A key design property of the fuzzer is its universality across draft versions. The tested servers implement different drafts:

\begin{itemize}
    \item The Python echo server (based on aioquic) implements draft-03
    \item The Rust echo server (based on wtransport) implements draft-09
\end{itemize}

Despite these differences, the same fuzzer targets all servers simultaneously. This is possible because:

\begin{enumerate}
    \item The QUIC stream mechanics are identical across all drafts: bidirectional streams prefixed with \texttt{0x41} and Session ID, unidirectional streams with type \texttt{0x54} and Session ID, and QUIC datagrams.
    \item The capsule format (type/length/payload) is the same across drafts; only the set of defined capsule types changes.
    \item Per RFC~9297, unknown capsule types must be silently ignored. Therefore, flow-control capsules (introduced in draft-09+) sent to a draft-03 server are valid test cases: a compliant server ignores them, while a non-compliant server that crashes reveals a bug.
\end{enumerate}
...
All test cases are recorded to a SQLite database (by default at \texttt{boofuzz-results/run\_<timestamp>.db}). The database uses the following schema:
...
\noindent where \texttt{conn\_idx} is a global connection counter (starting at~0) and \texttt{EVENT} is one of a defined set of event types:

\begin{itemize}
    \item \texttt{SERVER\_READY} --- server is listening (with \texttt{bind=host:port})
    \item \texttt{SESSION\_OPEN} / \texttt{SESSION\_CLOSE} --- session lifecycle events
    \item \texttt{RECV\_BIDI}, \texttt{RECV\_UNI}, \texttt{RECV\_DATAGRAM} --- data received on the respective transport
    \item \texttt{ECHO} --- echo response sent (with type and stream/session ID)
    \item \texttt{STREAM\_RESET} --- stream reset received (with error code)
\end{itemize}
...
\begin{enumerate}
    \item A Python echo server based on aioquic, implementing draft-03~\cite{w3c_echo_server}
    \item A Rust echo server based on the wtransport library, implementing draft-09~\cite{wtransport_github}
\end{enumerate}
